    ...
    jint obstacleCount, jintArray jox, jintArray joy)
{
    ...
    for (int i = 0; i < (int)obstacleCount; i++) {
        int cx = (int)oxArr[i] / 10;
        int cy = (int)oyArr[i] / 10;
    ...
    int sc = max(0, min(gw - 1, (int)startX / 10));
    int sr = max(0, min(gh - 1, (int)startY / 10));
    int ec = max(0, min(gw - 1, (int)endX / 10));
    int er = max(0, min(gh - 1, (int)endY / 10));
    ...
    for (size_t i = 0; i < path.size(); i++) {
        ...
        jint data[2] = {(jint)(path[i].second * 10 + 5), (jint)(path[i].first * 10 + 5)};
